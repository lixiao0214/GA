...
tournament <- function(originalparents, couplenum, subsetnum){
  ...
  getsubpars <- function(originalparents,subsetnum){
    ...
    index <- sample(1:oriparentnum, floor(totalrownum/subsetnum)*subsetnum, replace = FALSE)
    ...
    for (i in 1:ncol(indexmatrix)){
      selectedindex <- indexmatrix[i,colmaxindex[i]] 
    }
    return(originalparents[selectedindex])
  }
  for (i in 1:partitiontime){
    parentspool[i*subsetnum + 1: (i+1)*subsetnum] <- getsubpars(originalparents, subsetnum)
  }
  ...
  secondparent <- split(secondparent, row(secondparent))
  #combine firstparent and secondparent to form the list of paired parents
  newcouple <- mapply(c, firstparent, secondparent, SIMPLIFY=FALSE) 
  return(newcouple)
}
...
selectparents <- function(originalparents, couplenum, method, fitfun = NULL, subsetnum ){
  #originalparents is a casenum * variablenum matrix
  #fitness is a casenum * 1 vector , usually it's AICvalue
  #method is the method to select the parents
  
  fitness <- getfitness(originalparents, fitfun = fitfun)
  oriparentnum <- nrow(originalparents)
  switch(method, 
